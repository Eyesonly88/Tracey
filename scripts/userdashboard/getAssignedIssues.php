 ...
 */
	...
	if (!(empty($result))) {
		...
		foreach($result as $row){
			...
			$response = $response . '<td class="i_lhours" name="i_lhours" align="center"><a id="iloghours" rel="shadowbox;height=292;width=810" href="logissuehour.php?id=' . $row['IssueId'] . '"><input type="button"  style="width:80px; height:35px; border-width:1px;" id="logIssue-button'. $row['IssueId'] . '" value="Log Work" /></a></td>'; 
			$response = $response . '<td class="i_viewissue" name="i_viewissue_button' . $row['IssueId'] . '" id="' . $row['IssueId'] . '" align="center"><a id="view_issue' . $row['IssueId'] . '" rel="shadowbox;width=900" href="issue.php?id='. $row['IssueId'] . '"><input type="button"  style="width:80px; height:35px; border-width:1px;" id="viewIssue-button'. $row['IssueId'] . '" value="View" /></a></td>';
			$response = $response . '</tr>';
		}
		$response = $response . '</tbody>';		
	} 
